...
import type { OpenAPIVersion, OpenApiZodAny, OpenApiZodAnyObject, ParsingArgs } from "./interfaces";
...
function iterateZodObject({ zodRef, useOutput, hideDefinitions, openApiVersion }: ParsingArgs<OpenApiZodAnyObject>) {
 ...
 * @returns
 */
function parseString({ zodRef, schemas, openApiVersion }: ParsingArgs<z.ZodString>): SchemaObject {
...
function parseNumber({ zodRef, schemas, openApiVersion }: ParsingArgs<z.ZodNumber>): SchemaObject {
	const baseSchema: SchemaObject = {
...
function parseObject({
	...
	else if (zodRef._def.unknownKeys === "passthrough") additionalProperties = true;
...
function parseRecord({ zodRef, schemas, useOutput, openApiVersion }: ParsingArgs<z.ZodRecord>): SchemaObject {
	return merge(
		{
			type: typeFormat("object", openApiVersion),
			additionalProperties:
...
function parseBigInt({ zodRef, schemas, openApiVersion }: ParsingArgs<z.ZodBigInt>): SchemaObject {
	return merge(
		{
...
function parseBoolean({ zodRef, schemas, openApiVersion }: ParsingArgs<z.ZodBoolean>): SchemaObject {
	return merge(
...
function parseArray({
	...
	if (zodRef._def.minLength != null) constraints.minItems = zodRef._def.minLength.value;
	if (zodRef._def.maxLength != null) constraints.maxItems = zodRef._def.maxLength.value;
...
function parseLiteral({ schemas, zodRef, openApiVersion }: ParsingArgs<z.ZodLiteral<OpenApiZodAny>>): SchemaObject {
...
function parseUnion({
	...
	if (contents.reduce((prev, content) => prev && content._def.typeName === "ZodLiteral", true)) {
		...
		const type = literals.reduce(
			(prev, content) =>
				!prev || prev === typeof content._def.value ? typeof content._def.value : null,
			null as null | string
		);
...
export function generateSchema(
	...
	const schemas = [...(Array.isArray(metaOpenApi) ? metaOpenApi : [metaOpenApi])];
	try {
